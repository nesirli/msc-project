...
rule downsample_and_assemble:
    ...
    log:
        "logs/04_assembly/{sample}.log"
    shell:
        """
        # --- Downsample to /tmp (never touches data/processed/) ---
        DS_R1="/tmp/{wildcards.sample}_ds_1_$$.fastq.gz"
        DS_R2="/tmp/{wildcards.sample}_ds_2_$$.fastq.gz"

        python scripts/04_assembly.py downsample \
            --r1 {input.r1} --r2 {input.r2} \
            --out_r1 "$DS_R1" --out_r2 "$DS_R2" \
        ...
        reads_r1=$(gzip -dc "$DS_R1" | wc -l)
        reads_r2=$(gzip -dc "$DS_R2" | wc -l)
        min_reads=1000

        if [ $reads_r1 -lt $min_reads ] || [ $reads_r2 -lt $min_reads ]; then
            echo "ERROR: Insufficient reads for {wildcards.sample}: R1=$reads_r1 R2=$reads_r2 lines" | tee -a {log}
        ...
        # --- Cleanup: ds files and SPAdes tmp dirs all in /tmp, nothing left on disk ---
        rm -f "$DS_R1" "$DS_R2"
        rm -rf "$SPADES_TMP" "$SPADES_OUT" 2>/dev/null || true
        """
